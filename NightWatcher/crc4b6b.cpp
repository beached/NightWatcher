		...
		namespace medtronic {
			namespace crc4b6b {
				uint8_t crc8( uint8_t const msg[], size_t const & msg_size ) {
					...
					static_assert(sizeof_array( crc8_table ) == 256, "CRC8 Table must have a length of 256");
					uint8_t crc = 0;
				...
				uint16_t crc16( uint8_t const msg[], size_t const & msg_size ) {
					...
					uint16_t crc = 0xFFFF;
					for( size_t i = 0; i < msg_size; i++ ) {
						auto const cur_byte = static_cast<uint16_t>(msg[i]);
						auto const idx = static_cast<uint8_t>(crc >> 8) ^ cur_byte;
				...
				void encode_4b6b( uint8_t const in_message[], size_t const & in_length, uint8_t out_message[], size_t & out_length ) {
					...
					for( size_t i = 0; i < in_length * 2; ++i ) {
						...
						if( int_bits_available >= 8 ) {
							...
							int16_t mask = (1u << int_bits_available) - 1;
