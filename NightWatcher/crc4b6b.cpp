		...
		namespace medtronic {
			namespace crc4b6b {
				...
				uint16_t crc16( uint8_t const * const msg, size_t const & msg_size ) {
					...
					for( size_t i = 0; i < msg_size; i++ ) {
						auto const short_c = 0x00FF & static_cast<uint16_t>(msg[i]);
						auto const tmp = (crc >> 8) ^ short_c;
						crc = (crc << 8) ^ s_crc16_table[tmp];
					}
					return crc;
				}
