		...
		namespace medtronic {
			namespace crc4b6b {
				namespace {
					...
					inline void crc8_part( uint8_t const & value, uint8_t & result ) {
						result = s_crc8_table[result ^ value];
					}

					inline void crc16_part( uint16_t const & value, uint16_t & result ) {
				...
				daw::Array<uint8_t, 2> crc8( uint8_t const * const msg, size_t const & msg_size ) {
					static_assert(sizeof_array( s_crc8_table ) == 256, "CRC16 Table must have a length of 256");
