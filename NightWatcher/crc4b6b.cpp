		...
		namespace medtronic {
			namespace crc4b6b {
				uint8_t crc8( uint8_t const msg[], size_t const & msg_size ) {
					...
					static_assert(sizeof_array( crc8_table ) == 256, "CRC8 Table does not have 256 elements, something changed it");

					return daw::accumulate( msg, msg + msg_size, 0, []( uint8_t const & crc, uint8_t const & val ) {
				...
				uint16_t crc16( uint8_t const msg[], size_t const & msg_size ) {
					...
					static_assert(sizeof_array( crc16_table ) == 256, "CRC16 Table must have a length of 256");

					return daw::accumulate( msg, msg + msg_size, 0xFFFF, []( uint16_t const & crc, uint8_t const & val ) -> uint16_t {
				...
				namespace {
					uint8_t get_4b6b_symbol( uint8_t const & in_symbol ) {
						...
						static_assert(16 == sizeof_array( s_symbols ), "s_symbol must have 16 symbols.  Someone changed it");
						assert( in_symbol <= 0x0F, "The in_symbol should never be more than 0x0F.  The caller is broken" );
				...
				void encode_4b6b( uint8_t const in_message[], size_t const & in_length, uint8_t out_message[], size_t & out_length ) {
					daw::fill( out_message, out_message + out_length, 0 );
					out_length = 0;

					nibble_queue nq;
					bit_queue bq;

					daw::for_each( in_message, in_message + in_length, [&]( uint8_t const & val ) {
						nq.push_back( val, 2 );
						bq.push_back( get_4b6b_symbol( nq.pop_front( 1 ) ), 6 );
						bq.push_back( get_4b6b_symbol( nq.pop_front( 1 ) ), 6 );
